  ...
  public add():void{
    this.service.createArtikl(this.data).subscribe(
      {next: (data) => {
       ...
       error: (err) => {
        this.snackBar.open(`There was an error during POST request`,
          'Okay', {duration: 2500}
        )
  ...
  public update():void{
    this.service.updateArtikl(this.data).subscribe(
      {next: (data) => {
       ...
       error: (err) => {
        this.snackBar.open(`There was an error during PUT request`,
          'Okay', {duration: 2500}
        )
        console.log(err.message);
       }}
  ...
  public delete():void{
    this.service.deleteArtikl(this.data.id).subscribe(
      {next: () => {
       ...
       error: (err) => {
        this.snackBar.open(`There was an error during DELETE request`,
          'Okay', {duration: 2500}
        )
        console.log(err.message);
       }}
